...
# 文件重命名
echo "[3/8] 重命名文件..."
find loon mihomo -type f \( -name "*.conf" -o -name "*.txt" \) | while read -r f; do
    dir="$(dirname "$f")"
    base="$(basename "$f")"
...
# 重命名交付
echo "[7/8] 重命名剩余文件..."
find loon mihomo -type f -name "*.raw.list" | while read -r f; do
...
echo "Done -> ${REPO_DIR}/loon, ${REPO_DIR}/mihomo"